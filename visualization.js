...
const TIER_STATES = {
  green: {
    landColor:        [0x27, 0xAE, 0x60], // lush green
    ...
    particleDensity:  0,
    degradation:      0.0,  // 0 = pristine, 1 = fully degraded
...
export async function initPlanet(canvasEl) {
  ...
  console.info(`[CarbonMirror] Visualization: ${state.mode}`);
}
...
export function updatePlanet(kg) {
  ...
  // Update smog overlay for CSS fallback
...
async function initThreePlanet(canvasEl) {
  const { Scene, PerspectiveCamera, WebGLRenderer,
          ...
          TextureLoader, AdditiveBlending, FogExp2,
          Points, PointsMaterial, BufferGeometry,
          Float32BufferAttribute, BackSide, MeshBasicMaterial,
          Vector3 } = THREE;
...
function drawCanvas2D() {
  ...
  const oceanHex= rgbToHex([0x21, 0x80, 0xB9]);
  ...
  // Land masses (animated with rotation)
...
function drawLandMasses(ctx, cx, cy, r, color, degradation, angle) {
  // Simplified continent shapes using arcs + rotated transforms
  const continents = [
    { x: 0.15, y: -0.25, rx: 0.22, ry: 0.35 },  // Eurasia
    { x: -0.3, y:  0.1,  rx: 0.18, ry: 0.28 },  // Americas
    { x:  0.2, y:  0.3,  rx: 0.14, ry: 0.20 },  // Africa + Arabia
    { x:  0.4, y:  0.15, rx: 0.12, ry: 0.15 },  // South/SE Asia
    { x: -0.05, y: 0.55, rx: 0.09, ry: 0.07 }, // Antarctica
  ];

  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
  ctx.clip(new Path2D(`M ${-r} ${-r} A ${r} ${r} 0 1 1 ${r} ${-r} Z`)); // not actually needed as planet already drawn
...
/** Deterministic pseudo-random from seed (no Math.random dependency for Canvas 2D static elements) */
